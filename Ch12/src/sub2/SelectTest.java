	...
	public static void main(String[] args) {
		...
		try {
			...
			while(rs.next()) { // 커서를 다음 튜플(행)로 이동
				...
				System.out.println("생년월일 : " + rs.getString(3));	// 커서가 가리키는 튜플의 3번 컬럼값 문자열로 참조
				System.out.println("휴대폰 : " + rs.getString(4));	// 커서가 가리키는 튜플의 4번 컬럼값 문자열로 참조
				System.out.println("나이 : " + rs.getInt(5));			// 커서가 가리키는 튜플의 5번 컬럼값 문자열로 참조
			}
